...
function [img,hdr] = read_image(hdr,filetype,fileprefix,machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB,slice_idx)
   ...
   if hdr.dime.datatype == 1 | isequal(hdr.dime.dim(4:8),ones(1,5)) | ...
	(isempty(img_idx) & isempty(dim5_idx) & isempty(dim6_idx) & isempty(dim7_idx) & isempty(slice_idx))
      ...
      img_siz = io_GetNIIimageSize(hdr);

      [img count] = fread(fid, img_siz, sprintf('*%s',precision));
      if count~=img_siz
          fprintf('\nERROR reading NIFTI file: %i elements expected (precision = %s), but %i were read!\n', img_siz,precision,count);
          fprintf(' File was open with machine = %s\n',machine);
          fprintf(' File had hdr.dime.bitpix = %i\n',hdr.dime.bitpix);
      end
